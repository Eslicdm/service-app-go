...
// SecurityConfig holds JWKS configuration for JWT validation. Mirrors the
// Spring member-request-service SecurityConfig: POST /api/v1/member-requests/**
// is public; everything else requires a valid JWT.
...
// AuthMiddleware validates JWT tokens via JWKS. POST /api/v1/member-requests/**
// is public (Spring permitAll); other endpoints require authentication.
...
// isPublicEndpoint returns true for swagger/actuator/health and
// POST /api/v1/member-requests/** (Spring permitAll).
